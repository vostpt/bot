...
client.on('ready', () => {
  ...
  schedule.scheduleJob(rule, () => {
    axios.get('https://bot-api.vost.pt/getIF.php').then(({ data }) => {
      let respnovos = '';
      let respatualizados = '';
      let respnovosimportantes = '';
      let respatualizadosimportantes = '';
      data.forEach(element => {
        if (element.tipo === '1') {
          ...
          respatualizadosimportantes += `${element.ea} :track_next: ${
            element.e
          }**__`;
        } else if (element.tipo === '4') {
          respatualizadosimportantes += '\n';
          respatualizadosimportantes += `__**${element.id} - `;
          respatualizadosimportantes += `#IF${element.l},${
            element.s
          } - Subiu para `;
          ...
          respatualizarimportancia += `__**${element.id} - `;
          respatualizarimportancia += `#IF${element.l},${element.s} - `;
          respatualizarimportancia += `${element.ea} :track_next: ${
            element.e
          }**__`;
          client.channels
            .get('559054697944580098')
  ...
  const ruleAlerts = new schedule.RecurrenceRule();

  ruleAlerts.minute = new schedule.Range(0, 59, 10);

  schedule.scheduleJob(ruleAlerts, () => {
    axios.get('https://bot-api.vost.pt/getAvisos.php', {}).then(res => {
      let respnovos = '';
      let resptwitter = '';
      res.data.forEach(element => {
        ...
        if (element.icon === ':dash:') iconTipo = '🌬';
        else if (element.icon === ':sunny:️:thermometer:') iconTipo = '☀🌡';
        else if (element.icon === ':snowflake:️:thermometer:') iconTipo = '❄🌡';
        else if (element.icon === ':cloud_rain:') iconTipo = '🌧';
        else if (element.icon === ':fog:') iconTipo = '🌫';
        else if (element.icon === ':snowflake:') iconTipo = '❄';
        else if (element.icon === ':ocean:') iconTipo = '🌊';
        else if (element.icon === ':thunder_cloud_rain:') iconTipo = '⛈';

        let tipo = '';

        if (element.tipo === 'Precipitação') tipo = 'Chuva';
        else tipo = element.tipo.replace(' ', '');

        let inicio = '';
        let fim = '';

        if (
          moment(element.inicio, 'YYYY-MM-DD H:mm').format('YYYY-MM-DD') ===
          moment(element.fim, 'YYYY-MM-DD H:mm').format('YYYY-MM-DD')
        ) {
          if (
            moment(element.inicio, 'YYYY-MM-DD H:mm').format('YYYY-MM-DD') ===
            moment().format('YYYY-MM-DD')
          ) {
            inicio = `${moment(element.inicio, 'YYYY-MM-DD H:mm').format(
              'HH:mm',
            )}h`;
            fim = `${moment(element.fim, 'YYYY-MM-DD H:mm').format(
              'HH:mm',
            )}h de hoje,`;
          } else if (
            moment(element.inicio, 'YYYY-MM-DD H:mm').format('YYYY-MM-DD') ===
  ...
  schedule.scheduleJob({ hour: 0, minute: 0, second: 30 }, () => {
    ...
    let resp = '';
    let respsentido = '';
    axios.get('https://bot-api.vost.pt/getSismos.php', {}).then(res => {
      res.data.forEach(element => {
        if (moment(element.time).format('L') === date) {
          if (element.sensed === true) {
            ...
            respsentido += `**Sentido em ${element.local} com Int. ${
              element.degree
            }** ${element.shakemapref}`;
      ...
      if (respsentido !== '')
        client.channels
          .get('551435401697558528')
          .send(`***Sismos sentido dia ${date}:***\n${respsentido}`);
      if (resp !== '')
        client.channels
          .get('551435401697558528')
          .send(`***Sismos de ${date}:***\n${resp}`);
...
client.on('message', msg => {
  ...
  if (msg.content.startsWith(prefixHelp) && !msg.author.bot) {
    ...
    if (command === 'commands') {
      ...
      resp +=
        '**!all important** - *Mostra todas as ocorrências marcadas como importantes na ProCiv.*\n';
      resp +=
    ...
    if (command === 'acronimo') {
      if (args.length < 1) {
        msg.channel.send(
          `*Give me more data* para eu poder trabalhar, ${msg.author}!`,
        );
      }
      const argumento = args[0].toLowerCase();
      axios
        .get(`https://vost.mariosantos.net/api/acronym/${argumento}`)
        .then(res => {
          let resp = '';
          resp += '\n';
  ...
  if (msg.content.startsWith(prefix) && !msg.author.bot) {
    ...
    if (command === 'rcm') {
      if (args.length === 0) {
        msg.channel.send(`Falta o dia, ${msg.author}!`);
      }
      const argumento = args[0].toLowerCase();
      if (argumento === 'hoje') {
        msg.channel.send(
    ...
    if (command === 'all') {
      if (args.length === 0) {
        axios.get('https://bot-api.vost.pt/getAllProciv.php', {}).then(res => {
          let resp = '';
          let respimportantes = '';
          res.data.forEach(element => {
            if (element.o > 20 || element.t + element.a > 5) {
              ...
              respimportantes = `${respimportantes}#IF${element.l},${
                element.s
              } - `;
              respimportantes = `${respimportantes +
                element.o}:man_with_gua_pi_mao: `;
        ...
        if (argumento === 'links') {
          axios
            .get('https://bot-api.vost.pt/getAllProciv.php', {})
            .then(res => {
              let resp = '';
              let respimportantes = '';
              res.data.forEach(element => {
                if (element.o > 20 || element.t + element.a > 5) {
                  ...
                  respimportantes =
                    `${respimportantes}__**` +
                    `#IF${element.l},${element.s} - `;
                  respimportantes = `${respimportantes}https://fogos.pt/fogo/2019${
                    element.id
                  }**__`;
                } else {
                  resp += '\n';
              ...
              else msg.channel.send(':fire: ***Sem Ocorrências***');
            });
        } else if (argumento === 'human') {
          if (args.length < 2) {
            msg.channel.send(`Falta o numero de operacionais, ${msg.author}!`);
          }
          axios
            .get('https://bot-api.vost.pt/getAllProciv.php', {})
            .then(res => {
                ...
                if (element.o > args[1]) {
                  if (element.o > 20 || element.t + element.a > 5) {
                    ...
                    respimportantes = `${respimportantes +
                      element.o}:man_with_gua_pi_mao: `;
                    respimportantes = `${respimportantes +
                      element.t}:fire_engine: `;
                    respimportantes = `${respimportantes +
                      element.a}:helicopter: - `;
                    respimportantes = `${respimportantes + element.e}**__`;
                  } else {
                    resp += '\n';
                    resp += `${element.d} - `;
        ...
        } else if (argumento === 'ground') {
          if (args.length < 2) {
            msg.channel.send(
              `Falta o numero de meios terrestres, ${msg.author}!`,
            );
          }
          axios
            .get('https://bot-api.vost.pt/getAllProciv.php', {})
        ...
        } else if (argumento === 'air') {
          if (args.length < 2) {
            msg.channel.send(`Falta o numero de meios aereos, ${msg.author}!`);
          }
          axios
            .get('https://bot-api.vost.pt/getAllProciv.php', {})
            .then(res => {
        ...
        if (argumento === 'important') {
          axios
            .get('https://bot-api.vost.pt/getImportantIF.php', {})
            .then(res => {
              let resp = '';
              res.data.forEach(element => {
                resp += '\n__**';
      ...
      if (!args.length) {
        msg.channel.send(`Falta o numero da ocorrência, ${msg.author}!`);
      }
      const argumento = args[0].toLowerCase();
      if (argumento === 'id') {
        axios.get('https://bot-api.vost.pt/getAllProciv.php', {}).then(res => {
      ...
      if (argumento === 'vento') {
        ...
        axios
          .get(`https://bot-api.vost.pt/getWindy.php?id=${id}`, {})
          .then(res => {
            let resp = '';
            resp += '\n';
            resp += `${res.data.id} - `;
            ...
            if (resp !== '')
              ...
              msg.channel.send(
                ':wind_blowing_face: :fire: ***Sem Ocorrência***',
              );
          });
      }
      if (argumento === 'status') {
        axios.get('https://bot-api.vost.pt/getAllProciv.php', {}).then(res => {
          ...
          let respimportantes = '';
          let estado = '';
          if (args[1] === 'Conclusão') estado = '8';
          if (args[1] === 'Curso') estado = '5';
          if (args[1] === 'Despacho') estado = '4';
          if (args[1] === 'Resolução') estado = '7';
          if (args[1] === 'Vigilância') estado = '9';
          res.data.forEach(element => {
      ...
      if (argumento === 'distrito') {
        ...
        let estado = '';
        if (distritos === 'aveiro') estado = '1';
        if (distritos === 'beja') estado = '2';
        if (distritos === 'braga') estado = '3';
        if (distritos === 'bragança') estado = '4';
        if (distritos === 'castelobranco') estado = '5';
        if (distritos === 'coimbra') estado = '6';
        if (distritos === 'évora' || args[1] === 'evora') estado = '7';
        if (distritos === 'faro') estado = '8';
        if (distritos === 'faro') estado = '9';
        if (distritos === 'guarda') estado = '10';
        if (distritos === 'leiria') estado = '11';
        if (distritos === 'lisboa') estado = '12';
        if (distritos === 'portalegre') estado = '13';
        if (distritos === 'porto') estado = '14';
        if (distritos === 'santarém' || args[1] === 'santarem') estado = '15';
        if (distritos === 'setúbal' || args[1] === 'setubal') estado = '16';
        if (distritos === 'vianadocastelo') estado = '17';
        if (distritos === 'vilareal') estado = '18';
        if (distritos === 'viseu') estado = '19';
        axios
          .get(
            `https://bot-api.vost.pt/getIFDistrito.php?distrito=${estado}`,
            {},
          )
          .then(res => {
            let resp = '';
      ...
      if (args.length === 0) {
        let resp = '***Meteorologia:***';
        axios.get('https://bot-api.vost.pt/getIPMA.php?day=0', {}).then(res => {
          let count = 0;
          res.data.forEach(element => {
            count += 1;
            resp += '\n';
        ...
        if (argumento === 'tomorrow') {
          let resp = '***Meteorologia:***';
          axios
            .get('https://bot-api.vost.pt/getIPMA.php?day=1', {})
            .then(res => {
              let count = 0;
              res.data.forEach(element => {
                count += 1;
      ...
      if (args.length < 1) {
        msg.channel.send(`Falta a data, ${msg.author}!`);
      }
      const argumento = args[0].toLowerCase();
      const date = moment(argumento).format('L');
      let resp = '';
      let respsentido = '';
      axios.get('https://bot-api.vost.pt/getSismos.php', {}).then(res => {
        res.data.forEach(element => {
        ...
        if (respsentido !== '')
          msg.channel.send(
            `***Sismos sentidos dia ${date}:***\n${respsentido}`,
          );
        if (resp !== '') msg.channel.send(`***Sismos de ${date}:***\n${resp}`);
      });
    } else if (command === 'alerts') {
      let resp = '';
      axios.get('https://bot-api.vost.pt/getAlertas.php', {}).then(res => {
        res.data.forEach(element => {
          resp += `${element.local}\n`;
          element.alertas.forEach(alerta => {
            resp += `**${alerta.nivel}** // `;
            resp +=
              alerta.icon + alerta.tipo === 'Precipitação'
                ? 'Chuva'
                : `${alerta.tipo} // `;
        ...
        if (resp !== '') msg.channel.send(`***Alertas:***\n${resp}`);
      });
    }
  }
});
